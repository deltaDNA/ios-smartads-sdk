...
target 'MyApp' do
    pod 'DeltaDNAAds', '~> 1.9.0', :subspecs => ['AdMob','MoPub']
end
```
...
请确保你对空闲对象的充分引用，否则代理方法可能无法被调用。当你想要显示另一个广告时，你可以重新使用这个空闲对象并再次调用`-showFromRootViewController`或者创建另一个。
